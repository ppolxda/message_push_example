...
{
public:
    EventManager(const int32_t thread_interval = 1) : events_(), running_(false), thread_interval_(thread_interval) {}
    ~EventManager()
    {
        stop();
    }
...
public:
    ...
    bool emptyQueue() { return queue_.emptyQueue(); }

    void addCallback(std::shared_ptr<EventMessage> cb)
    {
        events_.push_back(cb);
        queue_.appendListener(EnumEventType::MatchStart, [cb](int)
                              { cb->matchStartCallback(); });
        queue_.appendListener(EnumEventType::MatchEnd, [cb](int)
                              { cb->matchEndCallback(); });
        queue_.appendListener(EnumEventType::CameraStream, [cb](const cv::Mat &leftFrame, const cv::Mat &rightFrame)
                              { cb->cameraStreamCallback(leftFrame, rightFrame); });
        queue_.appendListener(EnumEventType::BallPosition, [cb](const cv::Point2f &leftPos, const cv::Point2f &rightPos)
                              { cb->ballPositionCallback(leftPos, rightPos); });
        // queue_.appendListener(EventType::PredTrackBallPosition, [cb](const std::vector<cv::Point3f> &pos)
        //                       { cb->predTrackBallPositionCallback(pos); });
        // queue_.appendListener(EventType::RealTrackBallPosition, [cb](const std::vector<cv::Point3f> &pos)
        //                       { cb->realTrackBallPositionCallback(pos); });
        // queue_.appendListener(EventType::ShuttlecockPosition, [cb](const cv::Point3f &pos)
    ...
    void emit(EnumEventType type, const cv::Point2f &leftPos, const cv::Point2f &rightPos)
    {
    ...
    {
        if (type == EnumEventType::PredTrackBallPosition || type == EnumEventType::RealTrackBallPosition)
