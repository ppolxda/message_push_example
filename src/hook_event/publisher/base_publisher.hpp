...
public:
    ...
    {
        return publish(topic, reinterpret_cast<const unsigned char *>(message.data()), message.size());
    }

    // 发布消息到指定主题/队列
    virtual bool publish(const std::string &topic, const unsigned char *message, const size_t size) = 0;
