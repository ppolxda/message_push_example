...
struct KafkaPublisherConfig
{
    ...
    std::string acks = "all";
    std::string securityProtocol = ""; // "PLAINTEXT", "SSL", "SASL_PLAINTEXT", "SASL_SSL"
...
{
public:
    KafkaPublisher(KafkaPublisherConfig cfg) : producer_(nullptr), config_(cfg), running_(false)
    {
        start();
    }
    ...
    {
        // RdKafka::Topic *topic = RdKafka::Topic::create(producer_, topic, nullptr, errstr);
        // if (!topic)
        // {
    ...
    bool publish(const std::string &topic, const unsigned char *message, const size_t size) override
    {
        if (!running_.load() || !producer_)
            return false;

        RdKafka::ErrorCode resp = producer_->produce(
            topic, RdKafka::Topic::PARTITION_UA,
            RdKafka::Producer::RK_MSG_COPY,
            const_cast<unsigned char *>(message), size,
            nullptr, 0,
            0, nullptr, nullptr);
